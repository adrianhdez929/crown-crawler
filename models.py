...
class Peer(Model):
    ...
    @staticmethod
    async def dump(ip, port, proto, version, height, services, timestamp):
        try:
...
def test():
    ...
    for peer in peers.iterator():
        print("({}, {}, {}, {}, {})".format(peer.ip, peer.client_version, peer.last_height, peer.node_type, peer.last_timestamp))
    print(len(peers))
